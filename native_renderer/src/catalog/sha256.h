#pragma once
// Compact SHA-256 (FIPS 180-4), self-contained — used ONLY to verify catalog
// integrity against the emitted body hash. No external dependency (rule 20).
#include <cstdint>
#include <cstddef>
#include <cstring>
...
struct Sha256 {
    ...
    void final(uint8_t out[32]) {
        ...
        uint8_t zero = 0; while (buflen != 56) update(&zero, 1);
        uint8_t lenb[8];
        for (int i = 0; i < 8; ++i) lenb[i] = uint8_t(total >> ((7 - i) * 8));
        update(lenb, 8);
        for (int i = 0; i < 8; ++i) {
